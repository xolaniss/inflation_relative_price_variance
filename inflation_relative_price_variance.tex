...
to determine how the relationship has evolved and its implications for
monetary policy.
...
inflation targets from a 3- 6 per cent band in 2000 to a 4.5 per cent
...
relative price dispersion \citep{caglayan2003} and that the functional
form of relationship is U-shaped and dynamic
...
relative price variability and optimal inflation target with policy
...
and since they follow a one-sided dynamic pricing rule (S, s){[}1{]}
when setting prices, inflation reduces the optimal real price. Given
price adjustment costs differ across firms (e.g.~due to differences
administrative costs) price adjustment occurs infrequently and at
sporadic times leading to staggard price setting and consequently an
increase in relative price variability with latter reflects nominal
rigidities rather than a scarcity of resources. Signal extraction models
emphasize the role of inflation surprises in increasing RPD. In these
models, under the assumption of imperfect information, firms observe
their own prices immediately and only observe aggregate prices
imperfectly, therefore unexpected inflation o inflation volatility
reduces the informational content of prices as firms are unable to
distinguish between idiosyncratic relative (real) shocks and aggregate
(inflationary) shocks
...
inefficiencies generating adverse real effects. While menu cost models
predict a linear relationship, signal extraction models predict
...
suggesting the optimal inflation can be non-zero.
...
semi-parametric framework (Robinsons,1988) for South Africa between 2009
and 2025. We further extend our analysis further by estimating the
inflation-RPD nexus using a quantile-on-quantile regression since the
semiparametric estimation provides average inflation effects on RPD, the
quantile-on-quantile estimation shows how the state-dependant inflation
effect specific areas of the distribution of relative price dispersion.
We chose a post 2008 financial crisis period sample because inflation
was broadly within the target range and monetary policy credibility had
...
the 3-6 per inflation target range. Our sample includes 2025 with
structurally lower inflation which averaged 3.2 per cent after the SARB
communicated its preference for 3 per cent lower bound and the National
Treasury officially adopting the 3 per cent in November 2025
nevertheless, we do not specifically make inference with regards to this
as an extended post sample period is needed.
...
\citet{ndou2017} are there only papers that examined the inflation and
RPD relationship for an emerging market, South Africa. \citet{ndou2014}
focused estimating the disaggregated CPI components and found
heterogenous effects of these on RPD over 1980-2010 before and after the
adoption of the inflation target framework while \citet{ndou2017}
focused on RPD for services only during 1980-2011. This paper
contributes to the empirical literature by examining the headline
...
Findings \[xxxxxxxxxxx\] The rest of the paper is organised as follows.
Section 2 presents the data and methodology. Section 3 discusses the
main results, Section 4 discusses the robustness tests and Section 5
concludes the paper.
...
persistence in RPD. For robustness, we repeat this estimation using core
inflation (excluding food and non-alcoholic beverages, and fuel and
electricity) instead of headline inflation to account for the potential
volatility in these components.
...
automatically using a cross-validation method. For the full sample, the
relationship shows a U-shaped profile in line with the theoretical
predictions of \citet{head2005} and empirical evidence from
\citet{choi2010} amongst others; however, in our case, inflation has a
consistently positive effect on relative price dispersion across the
inflation range.

Specifically, Figure~\ref{fig-semi_parametric} shows small positive
effects on RPD for an inflation range of 2-3 per cent, moreover RPD is
at its lowest in this range suggesting this would be the optimal
inflation range as RPD is not amplified. For inflation rates above 3 per
cent but below 4 per cent, RPD rises but still at a relatively low
intensity compared to the rapid increase in RPD when inflation is above
4 per cent. For inflation rates above 4 per cent, RPD increases rapidly
and convexly implying high price misalignment, leading to inefficient
resource allocation which has an effect equivalent to a negative
productivity shock \citep{damjanovic2010}. These findings are similar
across both the Guassian and Epanechnikov kernels.

Interestingly, the sub-samples results show regime shifts in the
inflation and RPD relationship. Prior to 2017, the relationship was
hump-shaped and nonlinear and in the post 2017 period the relationship
was monotonic with a constant slope implying a relatively stable
inflation regime consistent with lower average inflation in this period
and that inflation consistently increases RPD. The regime dependant
nature of the inflation-RPD nexus is similarly to findings by
\citep{choi2011, baglan2016, hornstein2024}. However, in both
sub-samples, a low inflation of near 2 per cent still minimizes RPD as
RPD increases exponentially at high inflation rates. These results are
broadly consistent with predictions of the menu cost models. The optimal
inflation suggested by these results is broadly similar to estimates in
the literature which range from 1-3 per cent in the post 2010 studies
\citep{adam2024}.\footnote{Optimal inflation rate estimates varying
  depending on model extensions (see (Diercks, 2019)) for a
  comprehensive list for example studies that include frictions such as
  zero-lower bound constraint, downward wage rigidities and relative
  price trends generally find moderately positive optimal inflation
  rates.} The results generally affirm South Africa's decision to lower
...
nonparametric effect of unexpected inflation on RPD and the findings,
particularly of the full and pre-2017 samples, are consistent with the
predictions of the signal extraction theory. Positive and negative
surprises both increase relative price variability with near zero
inflation surprises minimizing relative price variability. Notably, in
the pre-2017 sample inflation surprises had symmetric effects but the
full sample shows that positive inflation surprises had a larger effect
or caused a steeper increase in RPD compared to negative inflation
surprises.

Broadly, for the full and pre-2017 samples, the intuition is that
inflation surprises create informational noise as a result firms confuse
aggregate price shocks with relative shocks leading to higher relative
price variability consistent with the misperception models
\citep{lucas1973, hercowitz1981, cukierman1983, barro1976}. The
post-2017 suggests a structural regime change in the relationship as the
effect of unexpected inflation on RPD is monotonic and linear but the
slope is negative implying price dispersion increases with inflation
uncertainty, contradicting the prediction of the misperception
theoretical models. Our post-2017 sample findings match the earlier
empirical evidence from \citet{reinsdorf1994}, which argued that this
negative relationship, arises from synchronized price adjustments by
firms reacting to aggregate inflation shocks rather than misperception
effects.

In our case, the post-2017 period is characterised by significant global
shocks to inflation from the COVID-2019 Pandemic in 2020 and associated
supply chain disruptions, the post-pandemic recovery induced energy
price shock in 2021,as well as the global food price shock after the
outbreak of the Russia-Ukraine war in 2021-2022. These common unexpected
shocks would've induced a similar price setting behavioural response in
the same direction therefore compressing price dispersion
\citep{nakamura2018}, which explains, in our view, the negative slope in
the post-2017 results in Figure~\ref{fig-unexpected_inflation}.
